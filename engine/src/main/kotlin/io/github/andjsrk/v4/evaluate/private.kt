...
fun LanguageType.privateGet(key: PrivateName): NonEmptyOrThrow {
    ...
    if (elem.data is AccessorProperty && elem.data.get == null) return throwError(TypeErrorKind.INVALID_PRIVATE_GETTER_ACCESS, key.description)
...
fun LanguageType.privateSet(key: PrivateName, value: LanguageType): EmptyOrThrow {
    ...
    if (isMethodProp) return throwError(TypeErrorKind.INVALID_PRIVATE_METHOD_WRITE, key.description)
    if (elem.data is AccessorProperty && elem.data.set == null) return throwError(TypeErrorKind.INVALID_PRIVATE_SETTER_ACCESS, key.description)
    return elem.data.setValue(this, key, value)
}

private fun LanguageType.privateAccessDenied(key: PrivateName) =
    throwError(TypeErrorKind.PRIVATE_NAME_ACCESS_FROM_OUTSIDE, key.description, display())
